...
def ground_marker(item: Any) -> Dict[str, Any]:
    # Handle both old (string) and new (dict) items
    ...
    for s in symbols:
        ...
        if g: groundings.append({"symbol": s, "info": g})
...
def ground_json(input_path: Path, output_path: Path) -> None:
    ...
    # Migrate main results first (ensure dict format)
    if "results" in data:
        data["results"] = [{"text": r, "evidence_text": "Retrospective migration"} if isinstance(r, str) else r for r in data["results"]]
